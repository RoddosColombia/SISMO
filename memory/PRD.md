...
- **Plan de Cuentas RODDOS — Conocimiento Base (✅ Febrero 2026)**: Colección `roddos_cuentas` en MongoDB con 49 cuentas reales de Alegra (codigo PUC + alegra_id interno + transacciones_tipicas + palabras_clave). `gather_accounts_context` usa `roddos_cuentas` primero (sin llamada API, <5ms) con detección de tipo de transacción (14 reglas). `execute_chat_action` traduce IDs inválidos via `roddos_cuentas` antes de ir a `/categories`. System prompt incluye PLAN DE CUENTAS RODDOS con IDs reales y asientos típicos (arriendo, honorarios, venta moto, etc.).: 10 funciones totales (incluyendo 4 nuevas: tributaria/flujo_caja/inventario/KPIs), semáforo `impuestos` dinámico vía fechas_dian DIAN, `/semaforo` retorna exactamente 5 keys, `/generar` incluye `diagnostico{bien,mal}` + `plan_accion` + `generado_en`, ICA 11.04‰ Bogotá. Test suites: `test4_suite.py` (53 casos) + `test_integration_cfo_test4.py` (13 casos).
