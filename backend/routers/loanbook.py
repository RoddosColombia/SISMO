...
@router.post("/admin/migrar-legacy-force")
async def migrar_legacy_force():
    """TEMP: Force migrate legacy loanbooks LB-2026-0001 to 0011 - NO AUTH REQUIRED FOR EMERGENCY"""
    try:
        # Find ALL legacy loanbooks by codigo pattern
        legacy_lbs = await db.loanbook.find({
            "codigo": {"$regex": "^LB-2026-00[0-1][0-1]$"}
        }).to_list(1000)

        print(f"[TEMP-MIGRATE] Found {len(legacy_lbs)} legacy loanbooks")

        updated_count = 0
        updated_lbs = []

        for lb in legacy_lbs:
            codigo = lb.get("codigo", "UNKNOWN")
            updates = {}

            # Calculate precio_venta from actual cuotas
            cuotas = lb.get("cuotas", [])
            cuota_inicial = lb.get("cuota_inicial", 0)
            ...
            if cuotas_ordinarias:
                ...
                updates["num_cuotas"] = num_cuotas
                updates["valor_cuota"] = valor_cuota
                print(f"[TEMP] {codigo}: precio_venta = {precio_venta}")
            ...
            if updates:
                ...
                if result.modified_count > 0:
                    ...
                    updated_lbs.append({
                        "codigo": codigo,
                        "precio_venta": updates.get("precio_venta"),
                        "fields_updated": len(updates)
                    })
                    print(f"[TEMP] {codigo}: Updated OK ({result.modified_count} docs)")
                else:
                    print(f"[TEMP] {codigo}: NO CHANGE ({result.matched_count} matched)")

        return {
            "status": "success",
            "total_found": len(legacy_lbs),
            "updated": updated_count,
            "updated_lbs": updated_lbs,
            "message": f"TEMP FORCE MIGRATED: {updated_count}/{len(legacy_lbs)} loanbooks"
        }

    except Exception as e:
        print(f"[TEMP-ERROR] {str(e)}")
